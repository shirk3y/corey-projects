...
class RRD(object):
    ...
    def graph(self, mins):       
        ...
        cmd.append('DEF:%s=%s:%s:AVERAGE' % (ds_name, self.rrd_name, ds_name))
        cmd.append('AREA:%s#FF6666:%s  ' % (ds_name, STAT))
        cmd.append('VDEF:%slast=%s,LAST' % (ds_name, ds_name))
        cmd.append('VDEF:%savg=%s,AVERAGE' % (ds_name, ds_name))
        cmd.append('VDEF:%smin=%s,MINIMUM' % (ds_name, ds_name))
        cmd.append('VDEF:%smax=%s,MAXIMUM' % (ds_name, ds_name))
        cmd.append('COMMENT:\\s')
        cmd.append('COMMENT:\\s')
        cmd.append('COMMENT:\\s')
        cmd.append('COMMENT:\\s')
        cmd.append('GPRINT:' + ds_name + 'last:last %.1lf%S    ') 
        cmd.append('GPRINT:' + ds_name + 'avg:avg %.1lf%S    ')
        cmd.append('GPRINT:' + ds_name + 'min:min %.1lf%S    ')
        cmd.append('GPRINT:' + ds_name + 'max:max %.1lf%S    ')
        cmd.append('COMMENT:\\s')
        cmd.append('COMMENT:\\s')
